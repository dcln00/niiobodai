...
- **Never use Pug dot or ID shorthand** (`.foo`, `div.foo`, `#hero`). Always write the element name explicitly with every class in a `class=` attribute: `div(class="flex items-center")`, `section(id="hero" class="py-16")`. When an element already has a `class=` attribute, merge the classes into it — one `class=` per element, before other attributes.
