...
<template lang="pug">
div
	section#hero(class="pt-32 pb-12 lg:pt-40 lg:pb-16 overflow-hidden relative")
		.container(class="space-y-6 max-w-5xl")
			...
			.space-y-4
				.flex(class="items-center gap-2")
					...
					span(class="font-mono text-xs text-brand-muted") // Instant Delivery
	...
	section#details(class="py-12 lg:py-20 bg-brand-surface-subtle")
		.container
			.grid(class="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-14 items-start")
				.content(class="lg:col-span-7 space-y-8")
					section-header(number="01" title="Overview" headline="What You Receive" description="Everything you need to step off the plane in America prepared for reality.")
					.space-y-5
						p(v-for="paragraph in product.details" :key="paragraph" class="text-base sm:text-lg text-brand-muted leading-relaxed") {{ paragraph }}
					.author-block(class="pt-4")
						store-author(:compact="true")
				.purchase-sidebar(class="lg:col-span-5 sticky top-28")
					.card(class="rounded-3xl lg:rounded-4xl bg-brand-surface border border-brand-black/8 p-8 lg:p-10 shadow-[0_8px_30px_rgba(23,22,21,0.05)] space-y-6")
						.flex(class="items-center justify-between pb-4 border-b border-brand-black/6")
							span(class="text-xs font-mono font-bold uppercase tracking-wider text-brand-muted") // Complete Toolkit
						...
						.price-section(class="pt-4 border-t border-brand-black/6 space-y-1")
							p(class="text-xs uppercase font-mono font-bold text-brand-muted tracking-wider") Total Price
						...
						.security(class="flex items-center justify-center gap-2 pt-2 text-[11px] text-brand-muted")
